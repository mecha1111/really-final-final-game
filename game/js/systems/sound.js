...
// ★ 2026-09-08: sfx_kill_soft_3.mp3·sfx_ui_click_1.mp3 두 파일 자체를 에셋에서
//   지웠다 — 이 목록 때문에 영구히 안 불릴 파일이라 배포본에 실려 나갈 이유가
//   없었다(무음에 가까운 결함 파일이라 되살릴 계획도 없다). 위 override 배열은
//   그대로 둔다 — 지금은 "파일이 없다"가 아니라 "이 인덱스는 원래 안 쓴다"가
//   유일한 진실이어야, 나중에 누가 같은 파일명으로 새로 채워 넣어도 이 코드가
//   무슨 뜻인지 헷갈리지 않는다.
...
//     그래서 같이 한 단계 낮췄다(0.55→0.3, 0.35→0.3).
//   - 2026-08-25(2차) copier 추가 인하: 그래도 여전히 크다는 피드백 — 실효 음량을
//     한 등급 더 낮췄다. 자폭음(선체감이 더 큰 쪽)을 우선해 두 단계, 등장음은
//     한 단계만 더 내려 지금 테이블에서 가장 조용한 축(등장음 최저치보다도 아래)에
...
const SFX_GAIN = {
  ...
  [SFX.ENTRANCE_PRINT]: 0.25, // 여전히 크다는 재피드백 — 추가 하향: 0.35→0.3→0.25
