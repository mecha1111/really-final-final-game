...
//   조작 설명은 이제 게임 시작 전 도우미 튜토리얼이 맡는다.
//   ★ 2026-09-08(재정정): 그 튜토리얼이 다시 옮겨갔다 — 처음엔 인트로(ui/intro.js)
//     안에 있었는데, 지금은 [게임 시작] 뒤(ui/gameOpening.js)로 옮겨가 타이틀
//     화면과 실제 판 사이에서 뜬다. 아래 "설정창 토글" 문단도 그에 맞춰 다시 고쳤다.
//   ★ 파일을 안 지우는 이유: 강아지 도우미(rover_idle.png)와 seenTips 세이브
//     필드를 그 튜토리얼이 그대로 물려 쓰고, __game.showTip 디버그 손잡이(main.js)로
//     여전히 이 사이드바를 불러볼 수 있다.
//   ★ 2026-09-07(추가): 설정창의 토글(#set-tutorial-on)은 "러버 힌트"에서
//     "시작 시 튜토리얼 보기"로 이름과 의미가 바뀌어, 지금은 이 파일이 아니라
//     게임 시작 오프닝의 강아지 튜토리얼 페이지(ui/gameOpening.js)를 켜고 끈다 —
//     다만 게이트 값(config.tutorial.enabled)은 여전히 공유하므로, 그 값이
//     꺼지면 showTip()도 여전히 조용히 무시한다(아래 함수 참고).
//
