 ...
 */
export function spotToRect(x, y, w, h, pad) {
  if (!holeEl) return;
  const p = pad ?? config.tutorial.spotPadPx;
  holeEl.style.left = `${Math.round(x - p)}px`;
  holeEl.style.top = `${Math.round(y - p)}px`;
  holeEl.style.width = `${Math.round(w + p * 2)}px`;
  holeEl.style.height = `${Math.round(h + p * 2)}px`;
 ...
 */
export function spotToWorld(cx, cy, w, h, pad) {
  // 논리 → #desktop(1920) 환산. getUiScaleFactor()가 그 반대 방향이라 역수를 쓴다.
  const k = 1 / getUiScaleFactor();
  spotToRect(cx * k - (w * k) / 2, cy * k - (h * k) / 2, w * k, h * k, pad);
}
