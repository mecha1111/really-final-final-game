...
// ★ "당첨"(아이폰 당첨!!! 여기 클릭) 문구는 뺐다 — 실제 스팸/피싱에서 그대로
//   쓰이는 "당첨" 낚시 문구를 게임 안에서까지 흉내 내는 건 그저 옛 데스크톱
//   개그(가짜 바이러스 경고·돌고래 드립 등)와 결이 다르다고 판단해 제거했다.
//   전용 config·에셋(아이콘·사운드·CSS 클래스)이 따로 없어(icon('star')·
//   .gagpop을 나머지 넷과 그대로 공유했다) 이 배열에서 항목 하나 빼는 것
//   외에 추가로 정리할 죽은 코드가 없다.
const GAG_LINES = [
 ...
 * failed(BSOD)·cleared(구간 클리어)·ending(완주 엔딩) 화면도 마찬가지 — 각각
 * .phase-failed/.phase-cleared/.phase-ending일 때만 .layer-bsod/.layer-cleared/
 * .layer-ending이 보인다.
 */
