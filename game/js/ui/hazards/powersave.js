...
// 화면이 서서히 꺼지고(1.2초에 걸쳐 암전), 마우스를 계속 움직이는 동안만 같은
// 속도로 다시 밝아진다(0.4초). 좌하단에는 XP 풍선 도움말이, 화면 정중앙에는
// "입력 신호 없음" XP 대화상자(모니터 OSD 패러디)가 뜬다.
//
...
//
// ★ 2026-09-10 재설계 — 실측으로 확인한 "설계 충돌"(버그가 아니었다). 회복
//   조건이 "커서 이동 속도(0.15초 이동평균) ≥ 200px/s"인데, 방해꾼을 클릭하려면
//   반드시 마우스를 목표로 옮겨야 하고 그 이동 자체가 곧 회복 조건이다 — 즉
//   클릭이 연출을 지우는 게 아니라, 클릭에 필요한 "이동"이 회복 조건이었다.
//   실측(적 0마리, 무입력): 1.5초에 dim 0.325, 3.0초에 dim 0.646(그 시절 최대
//   0.65)까지 올라간다. 그런데 같은 자리를 클릭 2회(=포인터 이동 포함)만 해도
//   직후 dim이 0.646 → 0.567로 오히려 밝아졌다. 플레이 중에는 조준 이동이
//   끊임없이 들어와 dim이 눈에 띄기 전에 계속 리셋됐고, 게다가 옛 dimInSec(3초)
//   으로는 durationSec 6초 안에 최대 어둠에 도달할 틈조차 거의 없었다. 그래서
//   이번 개편은 "암전을 훨씬 빨리(1.2초) 채우고, 회복도 짧게(0.4초, 대칭 유지)
//   만들어 어두워지는 장면 자체가 반드시 한 번은 보이게" 한다(config.hazard.
//   powersave의 dimInSec·wakeRecoverSec 주석에 이어서 적음). 클릭 자체는 여전히
//   회복 조건이 아니다 — 지금도 판정은 아래 update()의 inst.pointerDelta(이동
//   거리)만 보고 클릭 이벤트는 아예 보지 않는다, 그 규칙은 이번에 안 바꿨다.
//
...
//   조작 불능이 되어버려 "보기 힘들게"가 아니라 "못 하게"가 된다). 아래 새로
//   추가한 "입력 신호 없음" 대화상자도 예외가 아니다 — reboot의 .hz-win은
//   pointer-events:auto를 여는데(그 창은 버튼을 눌러 해제하는 방식이라 필요하다),
//   이 방해는 클릭으로 푸는 게 아니므로 대화상자에 pointer-events:none을
//   명시적으로 박아둔다(style.css의 .hz-powersave-osd 참고 — hz-win 클래스를
//   안 쓰는 이유도 그것이다).
//
...
registerHazard({
  ...
  mount(inst) {
    ...
    // 풍선 도움말과 "입력 신호 없음" 대화상자는 막 안에 같이 둔다 — 막이
    // 어두워질수록 같이 묻히면 안 되므로 불투명도는 막(--hz-dim)에만 걸고 나머지는
    // 자기 불투명도를 따로 갖는다(DOM 순서상 막보다 뒤에 와 그 위에 그려진다).
    //
    // ★ 2026-09-10 추가 — "입력 신호 없음" 대화상자. 화면이 서서히 꺼지는
    //   연출만으로는 "절전 모드"로 안 읽혀서(좌하단 풍선은 작고 눈에 잘 안
    //   띈다), 실물 모니터가 신호를 잃었을 때 띄우는 OSD 문구를 화면 정중앙에
    //   추가한다. 위치는 reboot(ui/hazards/reboot.js)가 쓰는 .hz-center를 그대로
    //   재사용하고, 창 뼈대도 기존 .win/.tbar/.ico/.t 어휘를 그대로 쓴다(새
    //   회색조 팔레트를 안 만든다는 원칙) — 다만 클래스는 hz-win이 아니라
    //   hz-powersave-osd다: hz-win은 pointer-events:auto를 여는데 이 대화상자는
    //   절대 클릭을 받으면 안 되기 때문이다(위 파일 상단 주석).
    el.innerHTML = `
      <div class="hz-dim"></div>
      <div class="hz-center">
        <div class="win hz-powersave-osd">
          <div class="tbar">
            <div class="ico">${icon('monitor', 18)}</div>
            <div class="t">입력 신호 없음</div>
          </div>
          <div class="hz-body">
            <div class="hz-row">
              <div class="hz-msg">
                <p>입력 신호를 찾을 수 없습니다.</p>
                <p class="hz-sub">절전 모드로 전환합니다.</p>
              </div>
            </div>
          </div>
        </div>
