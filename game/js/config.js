...
export const config = {
  ...
  enemy: {
    ...
    artHitbox: {
      ...
      // 2026-08-23: "X 버튼을 못 찾는다"는 피드백으로 ui/renderEnemies.js가 이 자리에
      // 원본의 작은 손그림 X 대신 훨씬 크고 대비 높은 진짜 버튼(빨강+흰 X)을 덧그린다
      // (drawPopupCloseButton). 그 버튼의 크기·위치가 곧 이 표의 값이다 — 그리기와
      // 판정이 정확히 같은 사각형을 쓰므로(히트박스를 시각 버튼보다 더 늘리지 않는다,
      // 요구사항) 아래 값을 원본 X의 중심은 유지한 채 확대했다(중심을 옮기면 버튼이
      // 원래 그림의 그 자리에서 벗어나 붕 떠 보인다).
    ...
    // 2026-08-23: 팝업 광고 X 버튼 인지도 개선. "어디를 눌러야 없어지는지 모르겠다"는
    ...
    popupCloseButton: {
      ...
      // 원본 손그림 X만 두고(아무것도 안 덧그림), 유저가 못 찾아 헤맬 때만 이
      // 버튼을 덧그린다. 손가락 아이콘은 제거(강조 X 자체로 충분하다는 판단).
      // 2026-08-25(4차): "몬스터마다 따로" 요구사항 — 몸통 오클릭 카운트가 전역
      // (state)이 아니라 방해꾼 인스턴스별(enemy.closeMisses, enemies/Enemy.js)
      // 이라, 화면에 팝업이 여러 마리 떠 있어도 헤맨 그 한 마리만 강조되고 나머지는
      // 원본 그대로다. 정확히 맞히면 그 방해꾼째 사라지므로 카운트도 같이 사라진다
      // — ui/renderEnemies.js의 drawPopupCloseButton 참고.
      missThreshold: 5,
