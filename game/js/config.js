...
export const config = {
  ...
  mouseCursor: {
    ...
    // hourglass 함정에 걸려 조작이 얼어있는 동안 쓰는 커서(ui/cursor.js).
    // ★ 2026-09-09: 예전엔 CSS 키워드 `wait`(=OS 기본 대기 커서)를 그대로 썼는데,
    //   맥에서는 무지개 원반, 윈도우에서는 작은 모래시계로 그려져 위 게임 커서
    //   (41x44)보다 눈에 띄게 작았다 — "지금 조작이 막혀 있다"는 상태가 커서
    //   크기부터 튀어서 오히려 안 읽혔다. 그래서 같은 41x44 캔버스에 그린 전용
    //   그림으로 바꾼다(assets/cursor/cursor-wait-44.png).
    //   · 크기: cursor-44.png와 같은 41x44 — 얼었을 때와 안 얼었을 때 커서가
    //     차지하는 자리가 똑같아서 크기가 널뛰지 않는다.
    //   · hotspot: 위 hotspotX/Y를 그대로 공유한다(아래 waitHotspot 없음 —
    //     일부러 안 만들었다). 두 그림이 같은 캔버스라 같은 화소가 같은 자리고,
    //     그래야 커서가 바뀌는 순간 포인터 위치가 안 튄다.
    //   · 경로가 '../'인 이유는 위 url과 완전히 같다(CSS 변수 안에서 풀린다).
    waitUrl: '../assets/cursor/cursor-wait-44.png',
  },
  ...
  enemy: {
    ...
    artHitbox: {
      ...
      // 2026-09-09(3차): 사방으로 0.02씩(원본 캔버스의 2%)만 더 넓혔다 — X 판정이
      // 빡빡하다는 피드백. 중심은 이번에도 그대로 두고 크기만 키운다.
      // ★ 이 이상 키우면 안 된다: "몸통을 누르면 실패한다"가 이 방해꾼의 규칙이라
      //   X 판정이 커질수록 그 규칙이 헐거워진다. 실측으로 확인한 전후 넓이
      //   (스프라이트 상자 대비, 그리드 160x160으로 훑어 센 값):
      //     popup:a  X 4.1% → 5.9%  (몸통 95.9% → 94.1%)
      //     popup:b  X 3.5% → 5.1%  (몸통 96.5% → 94.9%)
      //   즉 몸통은 여전히 94% 이상이라 "아무 데나 누르면 닫힌다"가 되지 않는다.
      //   실제 클릭으로도 몸통 4곳은 전부 그대로 실패했다.
      'popup:a': { l: 0.565, t: 0.045, r: 0.775, b: 0.225 }, // 상단 팻말의 X — 원본 중심 유지, 적당히 확대
      'popup:b': { l: 0.785, t: 0.31, r: 0.965, b: 0.49 }, // 우측 창의 X — 원본 중심 유지, 적당히 확대
    ...
    // fake_btn(업데이트 취소 버튼) 함정을 밟았을 때 뜨는 XP 시스템 대화상자
    // (ui/trapDialog.js). 벌칙(진행도 -10%)은 시트의 special_effect가 정하고
    // 여기는 "그 사실을 알려주는 창"의 표시 시간만 갖는다.
    trapDialog: {
      // 뜬 뒤 저절로 사라지기까지(ms). 클릭하면 그 전에도 닫힌다.
      // 길게 잡으면 화면 한복판을 오래 가린다(창이 클릭은 안 먹지만 시야는 가린다).
      holdMs: 1500,
    },
  ...
  hazard: {
    ...
    // ★ 2026-09-09: 여기 있던 rotationEnabled([화면 상하반전] 별도 토글)를 걷어냈다.
    //   접근성 목적으로 뒀지만 설정창에 토글을 하나 더 둘 만큼의 값이 아니었다 —
    //   상하반전은 이제 위 enabled([환경 방해])로 다른 방해들과 함께 켜고 끈다.
    //   ★ 세이브의 settings.rotationEnabled 필드는 남아 있다(core/save.js) —
    //     지우면 마이그레이션이 필요해서, 읽지도 쓰지도 않는 채로 그냥 둔다.
    // 동시 발동 상한. 지금은 1 — 여러 개가 겹치면 무엇을 먼저 해제해야 하는지
    // 판단이 안 서고, 해제 조작끼리도 서로를 가린다(예: 스크린세이버가 재부팅
    // 대화상자의 버튼을 덮는다). 나중에 상향 예정.
    maxConcurrent: 1,
    // ★ 2026-09-10 신설 — 구간(판)당 자동 스케줄 발동 횟수 상한. maxConcurrent가
    //   "동시에 몇 개"라면 이건 "판 전체에서 몇 번"이다 — 서로 다른 축이라
    //   maxConcurrent=1이어도 한 판에 여러 번(순차로) 터질 수 있었다(실측:
    //   보통 실력 판당 3~4회). 밸런스 재설계로 본편 등장 요소를 줄이면서(방해꾼
    //   7종+환경 방해 2종) 환경 방해 자체도 "구간마다 한 번" 정도로 눌렀다.
    //   systems/hazard.js의 scheduledFireCount가 이 값에 닿으면 새로 거는 것만
    //   멈춘다(이미 뜬 것·진행 중인 전조는 그대로 끝까지 간다) — startGame()마다
    //   resetHazards()가 0으로 되돌리므로 무한모드도 층마다(=판마다) 똑같이 1회다.
    maxPerStage: 1,
    // 구간 시작 후 이만큼(초)은 아무것도 안 터진다 — 시작하자마자 화면이 가려지면
    // "무슨 판인지" 파악할 틈이 없다.
    // ★ 2026-09-09 3 → 25(승인분). 3초는 방해꾼 배치조차 다 보기 전에 첫 방해가
    //   터질 수 있는 값이었다 — 구간 초반은 새로 풀린 방해꾼을 눈에 익히는
    //   시간이라, 환경 방해까지 겹치면 그 학습을 방해한다. 25초면 초반 리듬을
    //   한 바퀴 겪어본 뒤에야 첫 환경 방해를 만난다.
    graceSec: 25,
    ...
    intervalMinSec: 5,
    intervalMaxSec: 15,
    // ★ 2026-09-09 신설(승인분) — 구간 종료(state.timeLeft)까지 이 시간(초) 이내엔
    //   새로 발동시키지 않는다(systems/hazard.js의 updateHazards). 막판에 환경
    //   방해가 겹치면 정직하게 채운 할당량을 시간 안에 못 내는 억울함이 생긴다 —
    //   이미 떠 있는 것과 전조는 그대로 진행하고(방치해도 되돌릴 방법이 있다),
    //   "새로" 시작하는 것만 막는다.
    endGuardSec: 20,
    ...
    // ★ 2026-09-09 1.0 → 1.8(승인분) — 전조마다 "무엇이 오는지" 이름을 밝히는
    //   트레이 XP 풍선 도움말이 새로 붙는다(아래 각 방해의 telegraphHint). 1.0초는
    //   그 문구를 다 읽기엔 짧다 — 늘려도 "막을 수 없다"는 전조의 원칙은 그대로다
    //   (전조 중엔 클릭 판정 자체를 안 받으므로, 읽을 시간이 늘어도 회피 수단은
    //   여전히 없다).
    telegraphSec: 1.8,
    ...
    weights: {
      ...
      //   ★ 2026-09-09 8 → 14(승인분). durationSec을 3 → 5.0초로 다시 늘리면서
      //   (아래 flip.durationSec 주석) 8이던 시절의 "가끔 본다" 감각이 옅어질
      //   여지가 있어, 등장 자체를 더 자주 보게 올렸다. 정본 실측표는 위 값이
      //   8이었을 때 것이라 그대로 남겨두되, 14는 그 표를 다시 재기 전까지의
      //   승인된 값이다.
      flip: 14,
    },
    ...
    reboot: {
      ...
      //   안전한 선택지는 [나중에] 하나뿐이다.
      // ★ 2026-09-09 15 → 8(승인분) — 함정 버튼이 XP 실물의 "기본 버튼" 강조
      //   (.settings-btn-primary)를 그대로 쓰는 탓에 처음 보면 반사적으로 누르기
      //   쉽다. 처음 당했을 때의 벌칙이 너무 크면 "억울하게 크게 맞았다"만
      //   남는다 — 낮춰서 한 번은 배우는 비용으로 만든다.
      penaltyPct: 8,
      // 피해 HUD 캡션 앞에 붙는 원인 이름("재부팅 -8%") — 무엇에 당했는지 알려준다.
    ...
    // ★2026-09-09 재설계 — Mystify(다각형+잔상) 연출을 버리고 XP 기본 텍스트
    //   마퀴("화면을 보호하고 있습니다"가 화면을 가로지르는 그 화면 보호기)로
    //   갈아엎었다. Mystify는 화려해서 오히려 "장식"으로 읽혔다 — 마퀴는 순수
    //   검정 + 흰 글자뿐이라 "진짜로 화면이 잠겼다"는 느낌이 훨씬 강하다.
    //   ★완전 암전이다(예전의 dimOpacity로 방해꾼을 살짝 비치게 하던 타협을
    //   버렸다) — 이 화면 보호기는 "가려서 못 보게"가 아니라 XP 실물 그대로를
    //   재현하는 게 목적이고, 실물 마퀴는 뒤가 안 비친다.
    //   ★잠긴 동안 업로드가 깎이면 안 된다(요구사항) — 방해꾼 클릭 자체가
    //   불가능한 유일한 hazard라(막이 pointer-events:auto로 클릭을 통째로
    //   삼킨다) 그 사이 들어오는 적의 주기공격까지 그대로 맞으면 "못 하는데
    //   맞기만" 하는 상태가 된다. 그래서 이 hazard가 떠 있는 동안만
    ...
    screensaver: {
      ...
      // ── 마퀴 텍스트 ─────────────────────────────────────────────────────
      // 글자 크기(px). ★#desktop의 1920×1080 기준 좌표계 값이다(다른 hazard
      //   풍선·튜토리얼 스포트라이트와 같은 관례) — #desktop이 transform:scale로
      //   통째로 줄어들므로, 여기 숫자 하나만 정하면 1280~1920 어느 창 크기에서도
      //   화면에 차지하는 비율이 똑같다. 150 / 1080 ≈ 13.9% ≈ 요구사항의 14%.
      fontSizePx: 150,
      // 화면 오른쪽 끝에서 왼쪽 끝까지(정확히는 "화면 밖 오른쪽"에서 "화면 밖
      // 왼쪽"까지, 텍스트 폭까지 포함한 전체 이동거리 기준) 1회 횡단에 걸리는
      // 시간(초). 실물 마퀴 화면 보호기의 속도감과 같은 자릿수.
      traverseSec: 6.0,
      // ★부드럽게 흘리지 않는다 — 이 px 단위로 끊어서 이동한다(기준서의 계단식
      //   원칙). CSS transition이 아니라 dt 누적으로 위치를 계산해 값이 바뀐
      //   프레임에만 DOM을 쓴다(ui/hazards/screensaver.js의 update 참고).
      stepPx: 8,

      // ── 파훼 힌트 ───────────────────────────────────────────────────────
      // 이만큼(초) 지나야 힌트 문구가 뜬다. ★처음 이 시간 동안은 아무 안내도
      //   없다 — XP 실물 화면 보호기처럼 보이는 게 먼저고, 그 정적 자체가
      //   연출이다(요구사항). 너무 빨리 힌트를 띄우면 "화면 보호기"가 아니라
      //   "막힌 화면 + 안내문"으로 읽힌다.
      hintDelaySec: 4.0,
      hintFontSizePx: 24,
      // 화면 맨 아래에서 이만큼(px) 띄운 자리에 힌트를 둔다.
      hintBottomPx: 40,
    },
    ...
    powersave: {
      ...
      // ★ 2026-09-10 3 → 1.2(설계 충돌 해소) — 회복 조건이 "커서 이동 속도
      //   ≥ wakePxPerSec"인데, 클릭하려면 반드시 마우스를 목표로 옮겨야 하고
      //   그 이동 자체가 곧 회복 조건이다. 그래서 플레이 중엔 조준 이동이 계속
      //   들어와 dim이 눈에 띄기 전에 리셋됐다(실측: 적 0마리·무입력으로
      //   1.5초 dim 0.325 → 3.0초 dim 0.646인데, 같은 자리 클릭 2회(=이동 포함)
      //   직후 0.646 → 0.567로 오히려 밝아짐). 게다가 3초짜리 dimInSec은
      //   durationSec 6초 안에 최대 어둠에 도달할 틈이 거의 없었다. 1.2초로
      //   줄여 "어두워지는 장면 자체가 반드시 한 번은 보이게" 만든다.
      dimInSec: 1.2,
      // 가장 어두울 때의 검은 막 불투명도. 1이 아니라 0.65 — 완전 암전이면
      // 스크린세이버와 달리 해제 조작(마우스 이동)의 피드백조차 안 보인다.
      // ★ 2026-09-09 0.85 → 0.65(승인분) — 아래 실측표는 0.85 기준으로 잰 것이라
      //   "거의 안 보임" 절대 수치는 그대로 참고하되, 최대 어둠 자체가 낮아져
      //   방치해도 방해꾼이 완전히 안 보이는 지경까지는 안 간다. wakePxPerSec
      //   문턱값(연속 이동 측정 근거)은 밝기와 무관해 그대로 유효하다.
      maxOpacity: 0.65,

      // ★ 2026-09-09 재설계 — "즉시 꺼짐"을 없앴다.
      //   예전엔 회복이 1.6/초(=어두워지는 속도의 5.6배)라 완전 암전에서 밝아지는
      //   데 0.53초밖에 안 걸렸다. 그래서 마우스를 조금만 움직여도 화면이 곧바로
      //   100%로 돌아왔고, "서서히 어두워진다"는 연출 자체가 의미를 잃었다
      //   (어두워질 틈이 없으니 볼 일이 없다).
      //   지금은 회복과 감쇠를 같은 속도로 맞춘다 — 그래서 이 방해는 "한 번 흔들면
      //   끝"이 아니라 "계속 움직여야 겨우 유지된다"가 된다. 손을 멈추면 그 즉시
      //   같은 속도로 다시 어두워진다.

      // 밝아지는 데 걸리는 시간(초). 완전 암전(maxOpacity)에서 0까지.
      // ★ dimInSec과 같은 값 = 회복과 감쇠가 대칭이라는 뜻이다(위 주석).
      //   여기를 dimInSec보다 작게 두면 예전의 "흔들면 즉시 밝아짐"으로 되돌아간다.
      // ★ 2026-09-10 3 → 0.4 — 위 2026-09-09의 "대칭" 결정을 ★의도적으로 깬다.
      //   지금은 대칭이 아니다: 감쇠가 1.2초(0.54 알파/초), 회복이 0.4초
      //   (1.63 알파/초)라 ★회복이 감쇠보다 정확히 3배 빠르다.
      //   (2026-09-10 정정: 이 값을 넣은 커밋의 주석은 "0.4=0.4이라 대칭 유지"
      //    라고 적었는데 사실이 아니었다 — dimInSec은 1.2고 0.4가 아니다.
      //    숫자는 승인된 그대로 두고 설명만 바로잡는다.)
      //   왜 비대칭을 택했나: 2026-09-09의 대칭은 "계속 움직여야 겨우 유지된다"를
      //   노린 값이었지만, 실제로는 조준 이동만으로도 회복이 계속 들어와
      //   어두워지는 장면 자체를 못 보는 게 문제였다(위 dimInSec 주석의 실측).
      //   그래서 역할을 나눴다 — 감쇠(1.2초)는 "손을 떼면 금방 어두워진다"를,
      //   회복(0.4초)은 "움직이면 즉시 반응한다"를 각각 맡는다. 회복이 빨라도
      //   손을 멈추면 1.2초 만에 다시 최대 어둠에 닿으므로 "멈추면 안 보인다"는
      //   이 방해의 본체는 그대로다(옛 5.6배 비대칭과 달리 3배는 그 균형을
      //   무너뜨리지 않는다 — 그때는 회복 0.53초 대 감쇠 3초였다).
      wakeRecoverSec: 0.4,

      // 이 속도(월드 px/초) 이상으로 움직이면 "쓰는 중"으로 친다.
      // ★ 단위가 px/프레임이 아니라 px/초인 이유: systems/hazard.js가 넣어주는
      //   inst.pointerDelta는 "이번 프레임의 이동 거리"라, 프레임당으로 재면
      //   같은 손놀림이 120Hz 화면에서는 60Hz의 절반 거리로 잡혀 난이도가 화면
      //   주사율에 따라 달라진다(실제로 예전 값 3px/프레임이 그랬다 — 120Hz에서
      //   360px/초, 60Hz에서 180px/초로 두 배 차이). 초당으로 환산해서 비교하면
      //   어느 기기에서나 같은 손놀림이 같은 판정을 받는다.
      // ★ 값의 근거 — 실제로 플레이하며 잰 것이다(방해꾼을 향해 커서를 옮기고,
      //   조준하고, 진짜 마우스로 클릭하는 리듬을 문턱값별 10초씩).
      //   dim 0=밝음, 0.85=암전 / "거의 안 보임"=dim>0.6인 시간 비율:
      //
      //     문턱    커서를 세워 조준          목표 주위를 계속 흔들며 조준
      //     150    평균 0.63, 거의 안 보임 68%  평균 0.10, 거의 안 보임 0% (최대 0.32)
      //     200    평균 0.62, 거의 안 보임 71%  평균 0.12, 거의 안 보임 0% (최대 0.34)
      //     250    평균 0.71, 거의 안 보임 79%  평균 0.12, 거의 안 보임 0% (최대 0.39)
      //
      //   셋 다 "세우면 안 보이고 움직이면 안 어두워진다"는 성질 자체는 같다 —
      //   이 구간에서 문턱값이 정하는 건 "얼마나 빨리 움직여야 하는가"뿐이다.
      //   그래서 값은 연속 이동 측정(아래)으로 골랐다: 같은 속도로 6초 동안
      //   꾸준히 움직였을 때 화면이 유지되는지만 본 것이다.
      //     50·100·150px/초 → 계속 암전(0.85). 200px/초 → 밝게 유지(0.07).
      //     300·450px/초 → 완전히 밝음(0.00).
      //   즉 200이 "유지되기 시작하는 최소 속도"다. 더 낮추면 손목만 까딱해도
      //   유지되고, 더 높이면 그만큼 더 크게 휘저어야 한다.
      //   참고: 논리 해상도 폭이 960이라 200px/초는 화면 가로를 4.8초에 훑는
      //   느긋한 속도다 — 빠르게 휘두르는 게 아니라 "손을 놓지 않는" 정도.
      //
      //   ★ 이 표는 2026-09-09에 한 번 고쳐 적은 것이다. 처음 잰 값에는 계측
      //     오류가 있었다: 검증 하네스가 인트로 연출이 끝나기 전에 클릭을 보내는
      //     바람에(인트로 레이어가 화면을 덮고 있어 클릭이 게임까지 안 갔다)
      //     방해꾼이 한 마리도 안 죽었고, 그래서 "목표가 늘 있는" 비현실적인
      //     상황에서 잰 숫자였다. 그때는 250이 흔들어도 유지가 안 되는 것처럼
      //     보였는데, 제대로 재니 그건 방해꾼이 다 죽어 목표가 없는 동안 손을
      //     멈춘 탓이었다. 문턱값 200이라는 결론 자체는 그대로지만(연속 이동
      //     측정이 근거다), 근거로 적어뒀던 숫자는 위 표로 바로잡았다.
      wakePxPerSec: 200,

      // 속도를 재는 창(초). 이 시간만큼의 이동평균을 문턱값과 비교한다.
      // ★ 왜 순간값이 아니라 평균인가(실측으로 잡은 문제): inst.pointerDelta는
      //   pointermove 이벤트가 이번 프레임에 왔을 때만 0이 아니다. 마우스 보고
      //   주기(보통 125Hz)와 화면 주사율(여기 120Hz)이 어긋나면, 손은 일정하게
      //   움직이는데도 어떤 프레임은 0으로 잡힌다. 순간값으로 판정하면 그 0인
      //   프레임마다 화면이 어두워지는 쪽으로 새서, 문턱값 200으로 적어두고
      //   실제로는 280쯤 움직여야 유지되는 상태가 됐다(측정: 200px/초 연속
      //   이동인데 후반 평균 dim이 0.69로 거의 암전).
      //   0.15초 창으로 평균 내면 "이벤트가 몇 개 들어왔는가"와 무관하게 손의
      //   실제 속도가 그대로 판정에 들어간다 — 그래야 위 wakePxPerSec에 적은
      //   숫자가 곧 플레이어가 내야 하는 속도가 된다.
      //   너무 길게 잡으면(0.5초+) 손을 멈춘 뒤에도 한동안 밝은 채로 남아
      //   "멈추면 어두워진다"는 규칙이 흐려진다.
      wakeWindowSec: 0.15,
    },
    ...
    driver: {
      ...
      // ★ 2026-09-09(승인분): 이 값이 안전한 이유가 "작다"가 아니라 "이 파일이
      //   실제로 clamp한다"가 되도록, ui/hazards/driver.js가 이 값을 그대로
      //   안 쓰고 config.cursor.fakeDelayMax(1.3, trailMaxAgeSec보다 항상 작게
      //   설계된 값)로 한 번 더 눌러 쓴다 — 나중에 여기 숫자만 무심코 키워도
      //   "궤적 시작점에 멈춰있는" 고장으로 바로 이어지지 않는다.
      delaySec: 0.25,
      // ★ 2026-09-10: 지연을 "화면에 아무 변화가 없는데 손만 안 먹히는" 버그처럼
      //   보이지 않게, 실제 위치엔 고스트 커서(화살표+점선 테두리)를, 지연된
      //   위치엔 평소 커서를(기존 fakeCursor 그대로), 둘 사이엔 점선을 그어
      //   보여준다. 아래 두 값은 그 연출 전용 상수 — delaySec(판정 지연)과는
      //   무관하다.
      //
      // 상단 XP 풍선("디스플레이 드라이버 응답 지연")이 떠 있는 시간(초).
      // ★ 근거: telegraphSec(1.8초)+본 효과 발동까지의 텀보다 짧게, 그러나
      //   "떴다 사라졌다"를 알아챌 수 있을 만큼(1초 미만은 놓치기 쉽다)이라
      //   2초 안팎으로 잡았다 — 계속 떠 있으면 좌하단 본 풍선과 중복 정보라
      //   화면만 복잡해진다.
      toastSec: 2,
      // 고스트(실제 위치)↔지연 커서 연결 점선을 그리는 최소 거리(px, #desktop
      // 좌표계). 이보다 가까우면(정지 상태 등) 안 그린다 — 몇 px짜리 선은
      // 반올림 오차로 프레임마다 각도가 튀어 지저분하게 깜빡인다(요구사항).
      linkMinDistPx: 6,
    },

    // ── E. 화면 깨짐(ui/hazards/cracked.js) ──────────────────────────────────
    // CRT 신호가 깨진 화면. 시야만 가리고(클릭 판정은 그대로 캔버스로 통과)
    // durationSec 뒤 저절로 복구된다 — 해제 조작은 없다.
    //
    // ★ 2026-09-10 재작업. 예전엔 "흰 네모 블럭"으로 읽혔다. 원인은 폭이 아니라
    //   (폭은 그때도 3~8px로 이미 얇았다) 색과 움직임이었다:
    //     · kind 'stuck'이 순백(#ffffff) — 밝은 XP 배경 위에서 그냥 흰 막대다.
    //     · 열 전체가 같은 색으로 위아래를 관통 — 신호가 깨진 게 아니라 그려 넣은 막대.
    //     · 수평 밀림이 없어 "찢어진 신호"의 핵심 어휘가 통째로 빠져 있었다.
    ...
    cracked: {
      ...
      durationSec: 4.5,
      // ★세로 열 개수. 얇아진 만큼 개수를 늘린다 — 다만 아래 maxOccupancyRatio가
      //   총 점유폭을 먼저 자르므로, 이 값은 "최대 몇 개까지 시도하나"에 가깝다.
      colCountMin: 5,
      colCountMax: 9,
      ...
      colWidthMin: 3,
      colWidthMax: 9,
      // ★총 점유폭 상한(화면 폭 대비 비율). 열을 만들다 이 예산을 넘기면 거기서
      //   멈춘다 — 개수를 늘려도 화면이 통째로 가려지는 일이 구조적으로 없다.
      maxOccupancyRatio: 0.12,
      // ★열 하나를 세로로 몇 조각으로 끊을지. 조각 경계는 균등분할이 아니라
      //   난수다 — 균등하면 그 자체가 규칙적인 무늬로 읽힌다.
      segMin: 2,
      segMax: 5,
      // 조각마다 좌우로 이만큼(px) 민다. 이 밀림이 "신호가 찢어졌다"의 본체다.
      offsetMinPx: 2,
      offsetMaxPx: 12,
      // 밀림을 다시 뽑는 주기(초). ★보간하지 않는다 — 이 주기에 값이 통째로
      //   바뀌고 그 사이엔 고정이다(이 프로젝트의 계단식 원칙). 다시 그리는
      //   주기도 이 값 하나를 쓴다(매 프레임 clearRect 금지).
      offsetIntervalSec: 0.15,
      // 열 위/아래 끝을 마감하는 밝은 선의 두께(px). CRT 라인 느낌을 내는 1px.
      capLinePx: 1,
    ...
    // ★ 2026-09-09 전까지는 [화면 상하반전] 토글로 이것만 따로 끌 수 있었다.
    //   지금은 [환경 방해] 토글이 다른 방해들과 함께 끈다(위 hazard.enabled 주석).
    flip: {
      // 5구간(index 4)부터 — 마지막 유한 구간에서만 나온다.
      minStage: 5,
      // ★방치 시 이 시간(초) 뒤 원래대로 돌아온다. 8초 → 3초로 줄였다 —
      //   화면이 뒤집힌 채 8초는 "방해"가 아니라 그냥 못 하는 시간이었다.
      // ★ 2026-09-09 3 → 5.0(승인분). 위 weights.flip을 8 → 14로 올려 더 자주
      //   보게 하는 대신, 한 번 걸렸을 때는 조금 더 버텨야 풀리게 했다 — "자주
      //   오지만 각각은 가볍다"보다 "적당히 자주 오고 그때마다 확실히 방해한다"
      //   쪽으로 다시 조정한 것.
      durationSec: 5.0,
  ...
  // ★ 2026-09-09(재정정): 튜토리얼의 내용이 통째로 바뀌었다 — 그림이 붙은 설명
  //   5쪽에서, 판 위에서 도는 ★스포트라이트 시연 5단계로(ui/tutorial.js).
  //   이 토글의 의미("안내를 보여줄까")와 이름은 또 그대로다. 다만 켜졌을 때
  //   벌어지는 일이 다르다: 이제 판을 ★먼저 시작하고 그 위에서 설명한다
  //   (ui/gameOpening.js 상단 주석의 흐름 재전환).
  // ★ 꺼도 게임 시작 오프닝의 렉·로딩 연출(config.opening)은 그대로 재생된다 —
  //   그건 튜토리얼이 아니라 오프닝이라 이 토글의 대상이 아니다. 꺼진 채로는
  //   ★예전 흐름 그대로다: 로딩이 끝나는 그 자리에서 비로소 판이 시작된다
  //   (판을 먼저 시작하는 건 튜토리얼이 붙을 때뿐이다 — 게이트로 멈춰둘 이유가
  //   없는데 판만 먼저 시작하면 그 3초가 순수한 손해라서).
  ...
  // 별도로 config.opening에 있고, 그 아래 uploadCapPct/spotPadPx가 인게임
  // 튜토리얼(ui/tutorial.js) 몫이다.
  tutorial: {
    ...
    // ★ 이 튜토리얼은 판 "위"에서 돈다 — 진행바가 실제로 차오르고, 실제 방해꾼을
    //   실제로 클릭해서 없앤다. 아래 값들은 그 "실제"를 안전하게 만드는 울타리다.

    // 진행바 자동 증가를 멈추는 상한(%). 여기 닿으면 systems/upload.js가 더는
    // 안 올린다.
    // ★왜 필요한가: 100%에 닿으면 파일이 완성되며 다음 파일로 넘어가 버린다
    //   (systems/file.js의 completeFile) — 설명 도중에 화면이 통째로 바뀌고
    //   할당량까지 오른다. 그러면 "이 바가 뭔지" 설명하던 대상이 사라진다.
    // ★55인 이유: 가장 짧은 파일(소형 15초)이라도 상한까지 8초 넘게 걸려
    //   "차오르는 중"이 충분히 보이고, 4단계에서 실제 공격 한 방(-3%)을 맞아도
    //   0에 안 닿아 되돌아가는 게 눈에 남는다. 절반을 조금 넘겨 둔 것은 바가
    ...
    // ★튜토리얼이 끝나도 원복하지 않는다(요구사항) — 그대로 실전 시작 값이 된다.
    //
    // ★2026-09-10 재조정 — "반응이 느리다"는 신고를 원인부터 재현해 확인했다
    //   (추정 아님, Playwright로 프레임 단위 실측). 스포트라이트가 대상에
    //   닿는 시점과 state.uploaded가 실제로 오르기 시작하는 시점은 애초부터
    //   같은 프레임이었다 — 의심했던 스포트라이트 전환(0.26s)은 이 첫 등장에는
    //   적용되지 않는다(부모 레이어가 display:none→block으로 같은 프레임에
    //   같이 나타나 애니메이션이 걸릴 "이전 프레임"이 없다 — 실측: 구멍의
    //   computedStyle이 0px에서 목표값으로 프레임 사이에 그냥 점프했다). d990ba3의
    //   변경 감지 메모이제이션도 원인이 아니었다(lastRect가 null로 시작해 첫
    //   대입은 항상 그대로 나간다).
    //   ★진짜 원인은 화면 표시(ui/statusWindow.js)가 Math.floor(state.uploaded)로
    //   정수만 보여준다는 데 있었다 — 이건 실전에서도 필요한(콤보 수입의 소수점
    //   MB를 매 프레임 떨리게 안 보여주는) 표시 규칙이라 여기서 건드리지 않는다.
    //   옛 rate(1MB/초)로는 정수 자리가 1 올라가기까지 꼬박 1초가 걸려서, 그
    //   1초 동안은 스포트라이트가 이미 와 있는데 숫자만 "0"에 멈춰 있는 것처럼
    //   보였다 — 이게 실측한 "반응이 느리다"의 정체다.
    //   rate를 6으로 올려 정수가 처음 올라가기까지 걸리는 시간을 약 0.17초로
    //   줄였다(스포트라이트 도달과 거의 같은 프레임대) — sec는 2로 줄여 총
    //   증가량(+12MB, 183MB 할당량의 약 6.6%)이 옛 값(+3MB)보다 과하게 커지지
    //   않게 맞췄다.
    quotaTickRate: 6, // MB/초 — 정수 자리가 처음 올라가기까지 ~0.17초
    quotaTickSec: 2, // 이 초 이상 머물러도 더 안 오른다(위 rate와 곱해 총 +12MB)
  ...
  opening: {
    ...
    // ★fast(아래)도 이 문구를 그대로 재사용한다 — 시간표만 다르지 "무엇을
    //   보여주는가"는 같은 연출이라 문구를 따로 안 둔다.
    deadSuffix: ' (응답 없음)',

    // ★2026-09-10 신설 — 2회차 이상(세이브의 seenIntro=true)에 쓰는 단축
    //   시간표. 위 opening 본표는 1회차 전용으로 그대로 남는다(요구사항:
    //   "1회차는 현행 그대로"). ui/gameOpening.js가 hasSeenIntro()로 골라 쓴다.
    //   ★완전히 없애지 않는다(요구사항: "단축판도 컨셉의 일부") — 창이 뜨고
    //   (응답 없음)이 스치고 로딩바가 한 번 훑는 정도만 남기되, 의도적으로
    //   축소한 같은 연출이다.
    //   ★loadLowSec이 없다 — 단축판엔 강아지가 절대 안 뜨므로(1회차 전용)
    //   로딩바를 하단으로 비킬 자리를 만들 이유가 없다.
    //   ★목표 총 길이 1.2초 — 실측(assistSec + outAssistSec + outDimSec =
    //   0.80 + 0.0 + 0.25 = 1.05초)로 여유를 뒀다(프레임 드랍에 밀려도 넘지
    //   않게). 로딩바 애니메이션(.op-load .loadbar .blk, introSlide 1.5초
    //   1주기) 기준 dim~assist 구간(0.80-0.25=0.55초)이 "한 번 훑는" 정도다.
    fast: {
      hangSec: 0.0,
      deadSec: 0.1, // "스치는 정도"만 — 본표(0.5초)보다 훨씬 짧게
      loadOnSec: 0.2,
      loadDimSec: 0.25,
      assistSec: 0.8, // 이 시점에 곧장 finish()로 넘어간다(단축판엔 튜토리얼이 없다)
      outAssistSec: 0.0, // 강아지가 애초에 안 떴으므로 그 페이드아웃도 없다
      outDimSec: 0.25,
    },

    // ★2026-09-10 신설 — 렉·로딩 구간(강아지/튜토리얼이 뜨기 전) 동안 "아무 데나
    //   클릭하면 건너뛰기" 힌트(#op-skip-hint)가 뜨기까지(초). 1회차·단축판
    //   둘 다 공유하는 단일 값이다 — 처음부터 보이면 "프로그램이 막 실행된"
    //   착각이 덜 산다(요구사항이 명시한 "0.6초 뒤부터").
    skipHintSec: 0.6,
  },
  ...
  debug: {
    ...
    // 2026-09-09: 검증용으로 다시 true였던 걸(임시 커밋 14d80cc) 원복. 배포본에
    //   이 값이 true로 섞여 나가면 D키로 구간점프·밸런스 슬라이더가 열린다 —
    //   다시 켤 일이 있으면 이 값만 true로, 끝나면 반드시 여기로 되돌릴 것.
    enabled: false,
  },
