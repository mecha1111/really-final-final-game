...
export const config = {
  ...
  clearScreen: {
    ...
    tuckSec: 0.5, // 폴더 앞면 뒤로 빨려들어가며 사라지는 시간
    ...
    // 통째로 건너뛰고 장면2(스탯+버튼)로 즉시 넘어가버렸다 — "폴라로이드 연출이
    // 아예 안 보인다"는 신고가 재현된 원인이 바로 이것(구현 자체가 없었던 게
    // 아니라, 매번 열리자마자 건너뛰어졌던 것). 클리어 화면에 들어온 뒤
    // 이 시간(초) 동안은 클릭/키 건너뛰기 요청을 무시한다 — 요청 자체는 버리지
    // 않고 유예 시간이 끝난 뒤에 처리한다(ui/clearScreen.js).
    skipGraceSec: 0.9,
