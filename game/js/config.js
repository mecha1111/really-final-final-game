...
export const config = {
  ...
  hazard: {
    ...
    powersave: {
      ...
      // ★ 2026-09-10 3 → 0.4 — 위 2026-09-09 결정(대칭 유지)을 의도적으로
      //   뒤집는다. dimInSec을 1.2로 줄인 지금도 대칭(0.4=0.4)은 유지했으니
      //   "흔들면 즉시 밝아짐"으로 되돌아간 건 아니다 — 되돌아간다는 건 회복이
      //   감쇠보다 빨라지는 것인데, 둘 다 1.2에서 0.4로 같은 비율로 줄었을 뿐
      //   비율 자체(회복 = 감쇠, 1배)는 그대로다. 값을 줄인 이유는 dimInSec을
      //   3→1.2로 줄이며 감쇠가 빨라진 만큼, 회복도 원래 비율(대칭)을 지키려면
      //   같이 빨라져야 하기 때문이다 — dimInSec만 줄이고 wakeRecoverSec을 3에
      //   묶어두면 "어두워지는 건 순식간인데 밝아지는 덴 3초"가 되어 대칭이
      //   깨진다. 0.4초는 마우스를 움직인 순간 바로 반응이 보일 만큼 짧아
      //   "움직이면 밝아진다"는 해제 규칙의 피드백이 명확해진다.
      wakeRecoverSec: 0.4,
