...
export const config = {
  ...
  //   토글이 아무 동작도 안 하는 죽은 스위치가 됐다. 그래서 지금은 인트로의
  //   강아지 튜토리얼 페이지(ui/intro.js)를 보여줄지를 이 값이 정한다 — "안내를
  //   보고 싶은가"라는 상위 의미 자체는 그대로라 이름(enabled)과 세이브 필드
  //   (settings.tutorialEnabled)는 그대로 재사용했다(마이그레이션 불필요).
  // ★ 꺼도 부팅·바탕화면·클릭 연출(오프닝)은 그대로 재생된다 — 그건 튜토리얼이
  //   아니라 오프닝이라 이 토글의 대상이 아니다. 꺼진 채로는 커서가 클릭한 직후
  //   곧장 타이틀로 넘어간다(ui/intro.js의 buildSteps 마지막 단계).
  ...
  // ui/rover.js 상단 주석 참고). 강아지 튜토리얼(ui/intro.js)의 타이밍은
  // 별도로 config.intro에 있다.
  tutorial: {
  ...
  //   여기 한 곳에서만 정한다. 튜토리얼에는 [닫기]가 있어 시간 제한이 없다.
  intro: {
    ...
    tutorialSec: 7.8, // 강아지 튜토리얼(말풍선) 시작
