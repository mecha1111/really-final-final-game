...
export const config = {
  ...
  //   튜토리얼이 붙었지만 [게임 시작] 뒤로 옮겨갔다 — 아래 tutorialSec 주석과
  ...
  intro: {
    ...
    // ★이름은 tutorialSec이지만 지금은 곧장 타이틀로 넘어가는 시각이다(toTitle) —
    // 강아지 튜토리얼이 [게임 시작] 뒤(ui/gameOpening.js)로 옮겨가면서 이 구간
    // 끝에 더는 안 붙는다. 이름을 그대로 둔 이유는 "부팅~클릭 연출이 끝나는
    // 시각"이라는 의미 자체는 안 바뀌었고, 인트로 타임라인 안에서 유일하게
    // 시청자가 기다리는 마지막 지점이라는 위치도 그대로라서다.
    tutorialSec: 7.8,
