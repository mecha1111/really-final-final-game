...
export const config = {
  ...
  hazard: {
    ...
    cracked: {
      ...
      durationSec: 12,
      // 블록 개수 범위(요구사항의 "3~5개") — 매번 이 사이에서 무작위로 뽑는다.
      blockCountMin: 3,
      blockCountMax: 5,
      // 블록 한 변의 크기 범위(px, 논리 해상도 기준). 정사각은 아니고 이 범위
      // 안에서 가로/세로 각각 따로 뽑는다 — 전부 같은 비율이면 기계적으로 보인다.
      blockSizeMin: 70,
      blockSizeMax: 160,
      // 노이즈 무늬가 다시 뽑히는 주기(초). 매 프레임 다시 뽑으면(60fps) 눈이
      // 아플 정도로 어지럽다 — 이 정도 간격이라야 "지지직거린다"로 읽히지
      // "발작 유발 백색소음"이 안 된다.
      flickerIntervalSec: 0.15,
    },
