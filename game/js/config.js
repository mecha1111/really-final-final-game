...
export const config = {
  ...
  enemy: {
    ...
    // === popup 같은 "창 안의 버튼으로 닫기"형 방해꾼 ===
    // 닫기 버튼 히트박스 크기(px, 기준 해상도). 그려진 버튼에 맞춰 가로/세로를 따로 둔다
    // — 예전엔 정사각형 한 변(44)이었는데, 지금 그림의 버튼은 130x39로 가로가 훨씬 길어서
    // 정사각형으로 잡으면 버튼 좌우 끝을 눌러도 안 먹는다.
    // (시트의 hit_w/hit_h와 무관 — 그건 이 타입에서 안 쓰인다.)
    closeButtonW: 130,
    closeButtonH: 44, // 그려진 높이는 39.4 — 누르기 편하게 살짝 넉넉히
    // 닫기 버튼의 실제 중심 — 스프라이트 중심(x,y) 대비 비율(size_w/size_h에 곱함).
    //
    // ★ assets/enemies/popup/1,2,3.png의 잉크 픽셀을 직접 스캔해서 잰 값이다:
    //   128x128 캔버스 안에서 버튼은 x 31~94, y 71~98 (세 프레임 모두 동일) →
    //   스프라이트 박스(260x180) 기준 중심에서 (-3, +29), 크기 130x39.
    //
    //   이 값은 원래 옛 낱개 popup.png(260x180, 우상단에 X 아이콘이 그려져 있던 그림)에서
    //   잰 0.225 / -0.2917이었다. 아트를 손그림 폴더 프레임으로 갈아끼우면서 그 비율을
    //   그대로 뒀는데, 새 그림엔 X가 아예 없고 그 자리는 텅 빈 흰 여백이라 "눌러도 안
    //   죽는" 버그가 됐다(그림 위에 판정 사각형을 그려서 확인함). 지금 그림에서 실제로
    //   그려진 유일한 버튼은 아래쪽 가운데 "확인"이라 거기로 옮겼다.
    //   → popup 아트가 최종본으로 바뀌면 이 값을 반드시 다시 재야 한다. 그림에 없는
    //     자리를 가리키고 있어도 화면상으론 아무 티가 안 나고, 플레이할 때 "왜 안
    //     죽지?"로만 나타난다.
    closeButtonOffsetXRatio: -0.0117,
    closeButtonOffsetYRatio: 0.1602,
