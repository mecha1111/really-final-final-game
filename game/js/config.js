...
export const config = {
  ...
  hazard: {
    ...
    powersave: {
      ...
      //   조준하고, 진짜로 클릭하는 리듬을 10초씩 반복). 두 가지 조작 방식으로
      //   각각 재서 "벌은 아프고 대처는 가능한" 지점을 골랐다
      //   (dim 0=밝음, 0.85=암전 / "거의 안 보임"=dim>0.6인 시간 비율):
      //
      //     문턱    커서를 세워 조준            목표 주위를 계속 흔들며 조준
      //     150    평균 0.60, 거의 안 보임 66%   평균 0.23, 거의 안 보임 0%
      //     200    평균 0.73, 거의 안 보임 81%   평균 0.16, 거의 안 보임 0% (최대 0.45)
      //     250    평균 0.63, 거의 안 보임 62%   평균 0.37, 거의 안 보임 29% (최대 0.85)
      //
      //   200이 둘의 대비가 가장 선명하다: 커서를 세우면 화면이 실제로 안 보이고
      //   (81%), 조금씩이라도 계속 움직이면 절대 완전히 어두워지지 않는다(최대
      //   0.45). 250은 흔드는 것으로도 유지가 안 돼(29%가 암전) 조준을 포기하고
      //   휘저어야 하니 "할 만하다"가 깨지고, 150은 세워둬도 덜 어두워져 벌이
      //   약하다. 클릭 성공 횟수는 셋 다 10초에 8~10회로 같았다 — 어느 값에서도
      //   "못 하게" 되지는 않는다(이 방해의 절대 규칙, 파일 상단 주석).
