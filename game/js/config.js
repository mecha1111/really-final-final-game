...
export const config = {
  ...
  stage: {
    ...
    //     5구간(4): (신규 방해꾼 없음)                 / hazard: portrait
    //     무한(5+): 전부 활성
    //
  ...
  hazard: {
    ...
    //   있어야 한다. 끄면 "모니터 세로모드"(ui/hazards/portrait.js)가 아예 후보에서
    ...
    // ── F. 모니터 세로모드(ui/hazards/portrait.js) ──────────────────────────
    // XP 디스플레이 설정에서 화면 방향을 세로로 바꾼 것처럼 화면 전체가 90° 돈다.
    ...
    portrait: {
      ...
      minStage: 5,
      // 방치 시 이 시간(초) 뒤 원래 방향으로 돌아온다(요구사항의 "8초 경과").
      durationSec: 8,
      // 돌아가는 각도(도, 시계방향). CSS 전환과 좌표 역변환이 같은 값을 쓴다.
      rotateDeg: 90,
