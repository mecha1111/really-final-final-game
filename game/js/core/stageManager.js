...
export function releaseTutorial() {
  ...
  // ★2026-09-10 신설 — 업데이트 진행바(#up-bar = state.file.progress)를 새
  //   파일로 갈아 실전을 0%에서 시작한다. 튜토리얼 동안 이 바는 두 가지를
  //   겪었다: 1단계(b)에서 uploadAuto로 55%까지 차올랐고(config.tutorial.
  //   uploadCapPct), 4단계에서 실제로 얻어맞아 그중 일부가 깎였다. 어느 쪽도
  //   플레이어가 실제로 "해서" 만든 값이 아니라 대본이 보여준 것이므로, 실전은
  //   자기 파일을 처음부터 잡고 시작해야 한다.
  //   ★grantFile()은 완전히 새 파일 객체를 만든다(systems/file.js) — 그래서
  //   "4단계에서 깎인 분량을 되돌린다"가 아니다(그 교훈은 그대로 유효하다,
  //   위 시간 복구 주석과 같은 원칙). 옛 파일은 그냥 버려지고, 새 파일이
  //   자기 진행률 0에서 시작할 뿐이다 — 되돌릴 대상 자체가 없어진다.
  //   ★할당량(state.uploaded)은 안 건드린다 — 1단계(a)에서 "칠 때마다
  //   오릅니다"로 가르친 그 증가분은 grantFile()이 손대는 범위 밖이라 그대로
  //   남는다(config.tutorial.quotaTickRate/quotaTickSec 주석 참고).
  grantFile();
