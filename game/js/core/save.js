...
function defaultSave() {
  return {
    ...
    // ★ 2026-09-09: 사운드 세 값 100 → 50. 이펙트도 배경음도 아직 프리로드만 되고
    //   실제로 안 나는 상태라(위 index.html 설정창의 "사운드는 아직 없어요" 힌트
    //   문구 참고) 100%가 무엇을 뜻하는지 검증할 길이 없었다 — 붙이고 나서 볼륨을
    //   맞추기보다, 나중에 소리가 실제로 붙을 때 놀라지 않을 절반값을 기본으로 둔다.
    //   ★ core/state.js의 state.settings 초기값과 항상 같은 수를 들고 있어야 한다
    //     (그 파일 주석 참고) — 이 객체는 "새 세이브"의 모양이고 그쪽은 "세이브를
    //     아직 못 읽었을 때"의 모양이라, 둘이 갈리면 부팅 첫 프레임만 다른 볼륨이
    //     잠깐 보인다.
    //   ★ 구버전 세이브는 안 건드린다 — sanitize()는 raw.settings에 이미 적힌 값
    //     (옛 세이브는 100을 그대로 들고 있다)을 그대로 읽고, 이 기본값은 그 필드가
    //     아예 없을 때만(신규 세이브·손상된 세이브) 쓰인다. 이미 쓰던 사람의 설정을
    //     조용히 바꾸는 일은 없다.
    settings: {
      soundMaster: 50, soundSfx: 50, soundBgm: 50,
